...
- [x] **Papan Diskusi per Provinsi** — alternatif ringan dari ide "chat relawan" (chat real-time dinilai berat untuk shared hosting: butuh WebSocket/polling terus-menerus). Setiap relawan/admin bisa menulis & membaca pesan di ruang diskusi per provinsi (tabel `diskusi_posts`, scope `provinsi` = kode BPS 2 digit), tanpa keluar dari Dataraga. Halaman tidak auto-polling — cukup reload untuk pesan terbaru, jadi beban server sama seperti halaman listing biasa. Bisa hapus pesan sendiri; admin bisa hapus pesan siapapun (moderasi).
- [x] **Menu "Daftar Relawan" digabung jadi "Forum"** — menu sidebar terpisah "Daftar Relawan" dihapus, diganti "Forum" (mengarah ke Papan Diskusi). Halaman Forum dibagi layout 70/30: 70% kolom kiri untuk pesan diskusi, 30% sidebar kanan menampilkan daftar relawan yang terdaftar di provinsi yang sedang dipilih — supaya fungsi "siapa saja relawan di sini" dari halaman lama tidak hilang, cuma dipindah jadi bagian dari Forum. Rute/controller/view `relawan.index` lama tidak dihapus (cuma di-unlink dari sidebar), tetap bisa diakses langsung kalau dibutuhkan.
...
| 27 Juli 2026 | Tambah Papan Diskusi per Provinsi sebagai alternatif ringan dari ide chat relawan real-time |
| 27 Juli 2026 | Gabung menu Daftar Relawan ke Forum: layout 70/30 (diskusi kiri, daftar relawan per provinsi di kanan) |
