                            ...
                            @auth
                                ...
                                @if(auth()->user()->isAdmin() || auth()->user()->isRelawan())
                                    @if($club->status_validasi === 'pending')
                                        <button @click="selected = { id: {{ $club->id }}, nama: '{{ $club->nama_club }}', action: '{{ route('clubs.validate', $club) }}' }; verifyOpen = true" class="p-2 rounded-lg text-emerald-600 hover:bg-emerald-50 transition-colors" title="Verifikasi"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></button>
                                    @else
                                        <button @click="selected = { id: {{ $club->id }}, nama: '{{ $club->nama_club }}', action: '{{ route('clubs.cancel-validate', $club) }}' }; unverifyOpen = true" class="p-2 rounded-lg text-red-600 hover:bg-red-50 transition-colors" title="Batalkan Verifikasi"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></button>
                                    @endif
                                @endif
                            @endauth
                        </div>
