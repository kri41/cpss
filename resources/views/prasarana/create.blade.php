<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h2 class="text-2xl font-bold text-slate-800">Tambah Prasarana Baru</h2>
                <p class="text-sm text-slate-500 mt-1">Isi data prasarana olahraga dengan lengkap</p>
            </div>
        </div>
    </x-slot>
                ...
                <div class="p-6 sm:p-8">
                    <form method="POST" action="{{ route('prasarana.store') }}" enctype="multipart/form-data" class="space-y-8">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input id="latitude" type="number" step="any" name="latitude" value="{{ old('latitude') }}" class="mt-1 block w-full rounded-lg border-slate-300 focus:border-sky-500 focus:ring-sky-500 shadow-sm" placeholder="-6.123456">
                                    @error('latitude')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                                </div>
                                <div>
                                    <label for="longitude" class="block text-sm font-medium text-slate-700">Longitude</label>
                                    <input id="longitude" type="number" step="any" name="longitude" value="{{ old('longitude') }}" class="mt-1 block w-full rounded-lg border-slate-300 focus:border-sky-500 focus:ring-sky-500 shadow-sm" placeholder="106.123456">
                                    @error('longitude')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                                </div>
                            </div>
                            <div class="mt-4">
                                <label for="alamat" class="block text-sm font-medium text-slate-700">Alamat Lengkap</label>
                                <textarea id="alamat" name="alamat" rows="2" class="mt-1 block w-full rounded-lg border-slate-300 focus:border-sky-500 focus:ring-sky-500 shadow-sm" placeholder="Jalan, RT/RW, patokan">{{ old('alamat') }}</textarea>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                <label class="flex items-center p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50">
                                    <input type="checkbox" name="akses_disabilitas" value="1" {{ old('akses_disabilitas') ? 'checked' : '' }} class="w-4 h-4 text-sky-600 border-slate-300 rounded focus:ring-sky-500">
                                    <span class="ml-2 text-sm text-slate-700">Ramah Disabilitas</span>
                                </label>
                                <label class="flex items-center p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50">
                                    <input type="checkbox" name="akses_parkir" value="1" {{ old('akses_parkir') ? 'checked' : '' }} class="w-4 h-4 text-sky-600 border-slate-300 rounded focus:ring-sky-500">
                                    <span class="ml-2 text-sm text-slate-700">Akses Parkir</span>
                                </label>
                                <label class="flex items-center p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50">
                                    <input type="checkbox" name="akses_transportasi" value="1" {{ old('akses_transportasi') ? 'checked' : '' }} class="w-4 h-4 text-sky-600 border-slate-300 rounded focus:ring-sky-500">
                                    <span class="ml-2 text-sm text-slate-700">Akses Transportasi</span>
                                </label>
                                <label class="flex items-center p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50">
                                    <input type="checkbox" name="fasilitas_ruang_ganti" value="1" {{ old('fasilitas_ruang_ganti') ? 'checked' : '' }} class="w-4 h-4 text-sky-600 border-slate-300 rounded focus:ring-sky-500">
                                    <span class="ml-2 text-sm text-slate-700">Ruang Ganti</span>
                                </label>
                                <label class="flex items-center p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50">
                                    <input type="checkbox" name="fasilitas_tribun" value="1" {{ old('fasilitas_tribun') ? 'checked' : '' }} class="w-4 h-4 text-sky-600 border-slate-300 rounded focus:ring-sky-500">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                            ...
                            <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-sky-600 rounded-lg hover:bg-sky-700 focus:ring-2 focus:ring-sky-500 focus:ring-offset-2">Simpan</button>
                        </div>
                    </form>
                </div>
            </div>
